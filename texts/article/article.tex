...
O sistema implementado é dividido em 4 módulos:

\begin{itemize}
    \item {\textbf{Engenharia de dados:}} Módulo com o objetivo de executar processos de transformação e limpeza de dados.
    \item {\textbf{Workflow de IA:}} Módulo que executa um Pipeline de uma aplicação automatizada de IA, com etapas de preparação dos dados (Pré-processamento), treinamento (Processamento) e avaliação dos resultados (Pós-processamento) para a geração de um modelo final.
    \item {\textbf{Gerente Autonômico:}} Módulo com o \textit{loop} MAPE-K que controla o Workflow como elemento gerenciado para automatizar todas as suas etapas, com o objetivo de evitar com que perca-se tempo em execuções manuais que podem demorar dependendo do algoritmo e do conjunto de dados utilizado.
...
Para determinação do que vai ser executado no Workflow, algumas estratégias foram implantadas no Planejador:
...
\begin{itemize}
	...
	\item \textbf{Intervalo de resultados da análise}: Pelo mesmo motivo da estratégia anterior, métricas com resultados não confiáveis podem causar distorções a pontuação obtida no processo de análise. Para amenizar esas distorções, foi criado um intervalo de pontuação mínimo e máximo para determinar pontuações que podem ser consideradas confiáveis para avaliação.
\end{itemize}
...
Foram realizados 3 Estudos de Caso para determinar a viabilidade da solução, manutenções futuras e funcionamento do \textit{loop} MAPE-K na escolha de melhores opções em diferentes contextos. Em todos eles, o objetivo foi a obtenção de uma classificação de crédito (boa ou ruim), através de uma série de \textit{features} e utilizando diferentes conjuntos de dados. No caso de alterações no sistema, também foram contadas as linhas de código realizadas em cada alteração, para definir se tais alterações são simples de serem feitas.
